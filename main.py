...
SECRET_KEY = os.environ.get("HANGONG_SECRET_KEY", os.environ.get("PEIXUN_SECRET_KEY", "")).encode("utf-8")
if not SECRET_KEY:
...
def _parse_site_photo_paths(value):
    """Return the two persisted site-photo groups, tolerating legacy empty values."""
    groups = {"1": [], "2": []}
    if not value:
        return groups
    try:
    ...
    if not isinstance(decoded, dict):
        return groups
    for group in groups:
        paths = decoded.get(group, [])
        if isinstance(paths, list):
            groups[group] = [path for path in paths if isinstance(path, str)]
    return groups


def _remove_site_photo_paths(value, requested_value):
    """Remove only paths that already belong to this record and return the removed paths."""
    groups = _parse_site_photo_paths(value)
    try:
        requested = json.loads(requested_value or '[]')
    except (TypeError, json.JSONDecodeError):
        requested = []
    requested = set(requested) if isinstance(requested, list) else set()
    removed = []
    for group, paths in groups.items():
        ...
        for path in paths:
            if path in requested:
                ...
                kept.append(path)
        groups[group] = kept
    return json.dumps(groups, ensure_ascii=False), removed


async def _prepare_site_photos(photo_groups):
    """Validate uploads before mutating a record or replacing its main photo."""
    allowed_extensions = {'.jpg', '.jpeg', '.png'}
    prepared_groups = [[], []]

    for group_index, uploads in enumerate(photo_groups, start=1):
        for upload in uploads or []:
            ...
            if extension not in allowed_extensions:
                extension = '.jpg'
            prepared_groups[group_index - 1].append((extension, content))
    return prepared_groups


def _append_site_photos(record_id, prepared_groups, existing_value=None):
    """Persist already-validated site photos and return their JSON path mapping."""
    groups = _parse_site_photo_paths(existing_value)
    target_dir = os.path.join(UPLOAD_DIR, "site_photos", str(record_id))
    new_paths = []

    try:
        for group_index, photos in enumerate(prepared_groups, start=1):
            for extension, content in photos:
                os.makedirs(target_dir, exist_ok=True)
                path = os.path.join(target_dir, f"{group_index}_{uuid.uuid4().hex}{extension}").replace('\\', '/')
                with open(path, 'wb') as file:
                    file.write(content)
                new_paths.append(path)
                groups[str(group_index)].append(path)
    except Exception:
...
def _build_storage_cleanup_plan(cursor, start_date: str, end_date: str):
    ...
    cursor.execute(
        f"SELECT id, id_card, photo_path, word_path, site_photo_paths FROM records WHERE {date_filter}",
        (start_date, end_date)
    )
    ...
    for record in records:
        ...
        file_candidates.extend((path, 'site_photo') for paths in _parse_site_photo_paths(record['site_photo_paths']).values() for path in paths)
...
def init_db():
    ...
    try:
        ...
        pass
    try:
        cursor.execute("ALTER TABLE records ADD COLUMN site_photo_paths TEXT DEFAULT ''")
        conn.commit()
    except sqlite3.OperationalError:
        pass
    ...
    # 焊接技能实操考试记录：一场考试可留存多张过程照片。
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS welding_skill_exams (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        record_id INTEGER NOT NULL,
        user_id INTEGER NOT NULL,
        result TEXT NOT NULL,
        defect_issues TEXT DEFAULT '',
        photo_paths TEXT NOT NULL DEFAULT '[]',
        ndt_required INTEGER NOT NULL DEFAULT 0,
        ndt_status TEXT NOT NULL DEFAULT '',
        ndt_photo_paths TEXT NOT NULL DEFAULT '[]',
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    ...
    for column, definition in (
        ...
        try:
            cursor.execute(f"ALTER TABLE welding_skill_exams ADD COLUMN {column} {definition}")
        except sqlite3.OperationalError:
            pass
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_welding_skill_exams_record ON welding_skill_exams (record_id, created_at)")
...
def delete_record(record_id: int = Form(...), admin = Depends(get_admin_user)):
    ...
    # 1. 获取照片路径，以便随后进行物理删除
    cursor.execute("SELECT photo_path, site_photo_paths FROM records WHERE id = ?", (record_id,))
    row = cursor.fetchone()
    if not row:
        conn.close()
    ...
    photo_path = row[0]
    site_photo_paths = _parse_site_photo_paths(row[1])
    ...
    if photo_path and os.path.exists(photo_path):
        try:
            ...
            pass
    for paths in site_photo_paths.values():
        for path in paths:
            managed_path = _managed_upload_path(path)
            if managed_path:
                try:
...
async def api_ocr_idcard(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            # 仅在 Redis 与至少一个 OCR worker 都可用时投递，避免结果后端长时间重连。
            ...
            if not celery_app.control.ping(timeout=0.5):
                raise RuntimeError("没有可用的 OCR worker")
            task = ocr_idcard_task.delay(temp_path)
            result = task.get(timeout=15, interval=0.1)
        except Exception as celery_err:
            print(f"[OCR-Fallback] 队列不可用，立即启用本地识别: {celery_err}")
...
async def create_record(
    ...
    site_photo_group_1: list[UploadFile] = File(default=[]),
    site_photo_group_2: list[UploadFile] = File(default=[]),
    ...
    prepared_site_photos = await _prepare_site_photos((site_photo_group_1, site_photo_group_2))
    ...
    try:
        if prepared_site_photos[0] or prepared_site_photos[1]:
            site_photo_paths = _append_site_photos(record_id, prepared_site_photos)
            cursor.execute("UPDATE records SET site_photo_paths = ? WHERE id = ?", (site_photo_paths, record_id))
        conn.commit()
    except Exception:
        conn.rollback()
        try:
...
@app.post("/api/welding-skill-exam")
async def save_welding_skill_exam(
    record_id: int = Form(...),
    result: str = Form(...),
    defect_issues: str = Form(""),
    requires_ndt: str = Form("false"),
    photos: list[UploadFile] = File(default=[]),
    current_user = Depends(get_current_user)
):
    """保存一次焊接技能实操考试及其连续拍摄的过程照片。"""
    if current_user['role'] != 'admin':
        raise HTTPException(status_code=403, detail="仅管理员或二级管理员可以进行考试")
    result = (result or '').strip()
    defect_issues = (defect_issues or '').strip()
    needs_ndt = str(requires_ndt).strip().lower() in ('1', 'true', 'yes', 'on')
    if result not in ('qualified', 'unqualified'):
        raise HTTPException(status_code=400, detail="考试结果无效")
    if result == 'unqualified' and not defect_issues:
        raise HTTPException(status_code=400, detail="不合格时请填写缺陷问题")
    if result == 'unqualified':
        needs_ndt = False
    if len(defect_issues) > 1000:
        raise HTTPException(status_code=400, detail="缺陷问题不能超过1000个字符")

    uploaded_photos = [photo for photo in (photos or []) if photo and photo.filename]
    if not uploaded_photos:
        raise HTTPException(status_code=400, detail="请至少拍摄一张考试过程照片")
    if len(uploaded_photos) > 60:
        raise HTTPException(status_code=400, detail="一次考试最多上传60张照片")
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, user_id FROM records WHERE id = ?", (record_id,))
        record = cursor.fetchone()
        if not record:
            raise HTTPException(status_code=404, detail="人员记录不存在")
        ...
        try:
            ...
            for index, (extension, content) in enumerate(prepared_photos, start=1):
                path = os.path.join(photo_dir, f"{index}{extension}").replace('\\', '/')
                with open(path, 'wb') as file:
                    file.write(content)
                written_paths.append(path)
            cursor.execute('''
                INSERT INTO welding_skill_exams (record_id, user_id, result, defect_issues, photo_paths, ndt_required, ndt_status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                record_id, record['user_id'], result, defect_issues, json.dumps(written_paths, ensure_ascii=False),
                int(needs_ndt), 'pending' if needs_ndt else 'not_required', beijing_now().strftime("%Y-%m-%d %H:%M:%S")
            ))
            conn.commit()
        except Exception:
    ...
    return {"code": 200, "message": "焊接技能考试已保存", "photo_count": len(written_paths)}
...
async def complete_welding_ndt(
    ...
    if len(uploaded_photos) > 60:
        raise HTTPException(status_code=400, detail="一次探伤最多上传60张照片")
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('''
            SELECT id FROM welding_skill_exams
            WHERE record_id = ? AND user_id = ? AND result = 'qualified' AND ndt_status = 'pending'
            ORDER BY id DESC LIMIT 1
        ''', (record_id, current_user['id']))
        exam = cursor.fetchone()
        if not exam:
            raise HTTPException(status_code=404, detail="未找到待探伤的合格考试记录")

        photo_dir = os.path.join(UPLOAD_DIR, 'welding_skill_exams', str(record_id), f"ndt_{uuid.uuid4().hex}")
        written_paths = []
        try:
            os.makedirs(photo_dir, exist_ok=True)
            for index, (extension, content) in enumerate(prepared_photos, start=1):
...
async def update_record(
    ...
    site_photo_group_1: list[UploadFile] = File(default=[]),
    site_photo_group_2: list[UploadFile] = File(default=[]),
    removed_site_photo_paths: str = Form(""),
    current_user = Depends(get_current_user)
):
    if not validate_id_card(id_card):
        raise HTTPException(status_code=400, detail="身份证号码格式不正确")
    ...
    exam_project = (record['exam_project'] if exam_project is None else exam_project) or ''
    ...
    if is_over_week:
        if prepared_site_photos[0] or prepared_site_photos[1] or removed_site_photos:
            conn.close()
            raise HTTPException(status_code=400, detail="超过 7 天的记录需先完成修改审核，暂不能追加现场照片")
    ...
    if prepared_site_photos[0] or prepared_site_photos[1]:
        site_photo_paths = _append_site_photos(
            record_id, prepared_site_photos, site_photo_paths
        )
    ...
    conn.commit()
    conn.close()
    if old_photo_path and old_photo_path != photo_path and os.path.exists(old_photo_path):
        try:
            os.remove(old_photo_path)
        except OSError:
            pass
    for path in removed_site_photos:
        managed_path = _managed_upload_path(path)
        if managed_path:
            try:
                os.remove(managed_path)
...
@app.get("/welding-skill-exam", response_class=HTMLResponse)
def get_welding_skill_exam():
    headers = {
        "Cache-Control": "no-store, no-cache, must-revalidate, max-age=0",
        "Pragma": "no-cache",
        "Expires": "0"
    }
    with open("static/welding_skill_exam.html", "r", encoding="utf-8") as f:
        return HTMLResponse(content=f.read(), headers=headers)

@app.get("/ndt", response_class=HTMLResponse)
def get_ndt():
    headers = {
        "Cache-Control": "no-store, no-cache, must-revalidate, max-age=0",
        "Pragma": "no-cache",
    ...
    with open("static/ndt.html", "r", encoding="utf-8") as f:
        return HTMLResponse(content=f.read(), headers=headers)
