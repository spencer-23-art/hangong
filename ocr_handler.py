...
def ocr_idcard_process(image_path):
    ...
    # 最多两次候选裁切识别：文档裁切器 → OpenCV。避免低质量图片反复执行完整 OCR。
    print("[OCR-Debug] 裁切顺序: document_preprocessor -> opencv（最多两次候选）")
    evaluations = []
    selected = None
    ...
    if selected is None:
        print("[OCR-Debug] 两次候选均未完整识别，保留最佳结果")
